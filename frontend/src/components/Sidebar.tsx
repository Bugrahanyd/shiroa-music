...
import { 
  Music, Palette, Mic, ShoppingCart, Upload, Compass, Heart
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const themes = [
    { 
      ...
      storyEn: 'Dive into the depths of midnight creativity.'
    },
    { 
      id: 'light', 
      name: 'Day', 
      color: 'bg-gradient-to-br from-sky-400 via-blue-200 to-white', 
      storyEn: 'Embrace the vibrant energy of daylight.'
    },
    { 
      id: 'japanese', 
      name: 'Sakura', 
      color: 'bg-gradient-to-br from-pink-300 via-pink-200 to-purple-200', 
      storyEn: 'Experience the delicate beauty of cherry blossoms.'
    },
    { 
      id: 'neon', 
      name: 'Cyber', 
      color: 'bg-gradient-to-br from-purple-900 via-violet-800 to-fuchsia-900', 
      storyEn: 'Step into the electric future.'
    },
    { 
      id: 'sunset', 
      name: 'Sunset', 
      color: 'bg-gradient-to-br from-orange-500 via-pink-500 to-purple-600', 
      storyEn: 'Witness the magic of Los Angeles golden hour.'
    }
  ];

  const menuItems = [
    { icon: Compass, label: t('nav.home'), href: '/discover' },
    { icon: Music, label: t('nav.tracks'), href: '/tracks' },
    ...(user ? [
      { icon: Heart, label: t('nav.favorites') || 'Favorites', href: '/favorites' },
        ...
        <div className="absolute bottom-4 left-0 right-0 p-4 space-y-3">
          ...
          <div className="relative group">
            <div className="flex items-center gap-3 p-3 rounded-lg theme-hover opacity-50 cursor-not-allowed">
              ...
              {isOpen && <span className="theme-text-secondary">{t('nav.studio')}</span>}
            </div>
            {isOpen && (
          ...
          <div className="space-y-2 relative">
            <div className={`flex items-center gap-2 ${!isOpen ? 'justify-center' : ''}`}>
              ...
              {isOpen && <span className="text-sm theme-text-secondary">{t('sidebar.theme')}</span>}
            </div>
